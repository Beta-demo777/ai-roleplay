...
export default function SearchInput({value, onChange, placeholder, id, className = ''}: SearchInputProps) {
  return <div className={`relative ${className}`}>
    ...
    <input
      ...
      className="h-9 w-full rounded-lg border border-zinc-800 bg-zinc-950/80 pl-9 pr-9 text-xs text-zinc-200 outline-none transition placeholder:text-zinc-600 focus:border-cyan-500/50 focus:ring-1 focus:ring-cyan-500/10"
    />
