...
export default function PersonasDashboard({personas, activePersonaId, onSave, onDelete, onSetActive, middlePanelWidth, onMiddlePanelResizeStart}: PersonasDashboardProps) {
  ...
  return <div className="management-root">
    ...
    <main className="management-workspace">
      <header className="management-header"><div className="flex items-center gap-2 text-sm font-semibold"><UserRound size={15} className="text-cyan-400" />{isNew ? '创建我的人设' : `编辑人设 · ${draft.name}`}</div>{selected?.id === activePersonaId && <span className="text-[10px] text-emerald-400">当前默认人设</span>}</header>
      <form onSubmit={submit} className="management-content max-w-3xl space-y-5">
        ...
        <div className="flex items-center justify-between border-t border-zinc-800 pt-5"><div>{!isNew && selected && personas.length > 1 && <button type="button" onClick={() => selected.id && window.confirm(`确定删除人设「${selected.name}」吗？相关对话将改用当前人设。`) && onDelete(selected.id)} className="ui-button-danger"><Trash2 size={13} />删除人设</button>}</div><div className="flex gap-2">{!isNew && selected && selected.id !== activePersonaId && <button type="button" onClick={() => selected.id && onSetActive(selected.id)} className="ui-button-secondary border-emerald-500/20 text-emerald-400 hover:bg-emerald-500/10">设为当前人设</button>}<button type="submit" className="ui-button-primary px-5"><Save size={13} />保存人设</button></div></div>
      </form>
    </main>
  </div>;
}
