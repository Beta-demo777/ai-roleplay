...
AI 角色、对话场景、多套用户人设、对话线程和消息由 FastAPI 持久化到 PostgreSQL。对话线程可分别绑定场景和用户人设，模型请求会依次编排角色设定、场景设定和该线程绑定的人设。首次升级到数据库版本时，前端会自动将当前浏览器中的本地数据导入数据库；之后数据库作为主数据源，localStorage 仅作为后端暂时不可用时的本地缓存。数据库迁移由 Alembic 管理，并在后端容器启动前自动执行。
